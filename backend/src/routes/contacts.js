...
export function contactRoutes(app) {
    ...
    app.get("/", async (request, reply) => {
        ...
        const { linkedinUrl } = request.query;
    ...
    app.post("/", async (request, reply) => {
        ...
        if (!profile)
            return reply.status(404).send({ error: "Profile not found" });
        const { linkedinUrl, name, headline, company, position, location, avatarUrl, summary, tags, } = request.body;
        if (!linkedinUrl || !name) {
